namespace Algoritmo_Prim_Denso
{
        ...
        class Grafo
        {
            ...
            private void Preencher()
            {
                ...
                for (int i = 1; i <= Vertices; i++)
                {
                    ...
                    {
                        if (qtdAresta < 40) //máximo de 40 arestas
                        {
            ...
            private int VerticeMinimo(int[] peso, bool[] inserido)
            {
                ...
                {
                    if (inserido[vertice] == false && peso[vertice] < arestaMinima) // se o vértice não estiver na AGM e a aresta for mínima
                    {
                        arestaMinima = peso[vertice]; // recebe o peso da menor aresta
            ...
            public int[,] Prim()
            {
                ...
                for (int i = 0; i < Vertices - 1; i++) //repete até existir N-1 vértices
                {
                    ...
                    for (int j = 1; j <= Vertices; j++) //percorre todos os vértices adjacentes a v, que não estão na AGM
                    {
